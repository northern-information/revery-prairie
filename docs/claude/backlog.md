...
referenced from `CLAUDE.md`. read when picking up a backlog item, editing `docs/backlog.yaml`, or working with the kanban TUI.

terminal kanban for the backlog item sequencing — the 13-feature rollout described in `docs/backlog-thinktank-v3.md`. v3 is the doctrine (locked design decisions, vocabulary, cosmology, sequence + dependency graph). `docs/backlog.yaml` is the running state (status, spec/plan/pr links, notes). the dashboard renders the YAML as a TODO / NEXT / IN PROGRESS / SHIPPED kanban so the next unblocked feature is always one glance away.

`npm run backlog` launches it. lives at `tools/backlog-tui/` (Ink TUI, run via `tsx`). dev-only — not bundled with the game, not in any tsconfig project, eslint-ignored.
...
3. once the spec and plan land, set `spec:` / `plan:` on the feature in `docs/backlog.yaml` and flip `status: in-progress`.
