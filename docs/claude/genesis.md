...
the bottom-right of the screen is the only HUD element: a tilde-prefixed year readout (`formatYearsAgo`) that reads `~13.8B years ago` at cosmic origin and decays through `~M years ago` and `~K years ago` bands to `~now` as gameplay begins. precision is banded — geology-citation register, never decimal-precise — and the formatter rounds the underlying lerp into honest steps. one `<p>`, no caption, no progress bar, no percentage indicator. consumer-electronics gestures (boot animations, splash screens, loading prefixes) are banned by the diegetic test locked in precis-thinktank-v4 round 9.
