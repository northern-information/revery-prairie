...
What follows is the rounds. Decisions are stated as decisions. Open questions are flagged. v4 is _additive_ to v3 — it does not invalidate v3 unless a round here explicitly amends it. Items that should propagate to `docs/backlog-thinktank-v3.md` are tagged `**Amendment to v3:**`.
...
- The activity event log is **deleted with no replacement** (resolved in round 0 of this v4 arc; spec is `#14` in `docs/backlog.yaml`). No toast, no chime, no ambient ticker. The world communicates by being looked at.
...
- **The `size` field is removed from `docs/backlog.yaml` entirely, retroactively.** Sizing was a vestige of human estimation; in the current authorship environment it is not load-bearing. Historical sizes remain in git history only.
...
**Tracked as:** `#14` in `docs/backlog.yaml`. Starts the new convention — non-RP-numbered backlog items get the next available integer id and become "story numbers" alongside the backlog item items.
...
> - Soil — should leak via depletion. Same patch planted three seasons running gets thinner. Rotation, fallow, nitrogen-fixing crosses. _Trifolium repens_ is already a nitrogen fixer in real life — the backlog item-correct game has encoded this in its species choices; we just haven't built the leak yet.
...
- **Soil depletion** — small spec, no deps beyond what is shipped. Could be added to `docs/backlog.yaml` as soon as the room wants to commit to it.
...
`coyote.ts:296-317` — when the coyote's pack fills up, the coyote walks to Gron and drops items near him. Gron is being used as a dump-target for inventory overflow. Mechanistically convenient. Cosmologically suspicious. _Why does the coyote bring things to Gron?_ The doctrine-aligned answer requires thought; defer; do not change in this round.
...
- **Meta-amendment to thinktank convention: the `size` field is removed from `docs/backlog.yaml` entirely, retroactively.** All 29 existing entries have the field stripped; the locked field order updates to omit it. Sizing was a vestige of human estimation and is no longer load-bearing. Historical sizes remain in git history.
...
- **Amendment to thinktank convention:** `size` field deleted from `docs/backlog.yaml` across all entries, retroactively.
