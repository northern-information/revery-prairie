# Sequencing the backlog: a three-PM debate

## Context

The précis (referenced as `docs/design.md`) lays out an ecological roguelike about stewardship and deep time — three nested loops (ruin expedition → ecological stewardship → the Revery), phenological seasons, partially-legible genetics, an evolving invasive biome, and failure as transformation.
...
- `docs/design.md` does not currently exist in the repo. Add it (or confirm where the canonical text lives) before #0 ships, since later specs will need to cite it.
...
_The plan had been approved and #0 was about to be picked up when Delta surfaced one more thing. The four reconvened._

> **Delta:** What if the egregores grow during winter?

> **Astrid:** _(beat)_ That changes the cosmology. If the egregores grow in the dormant months — when the natives sleep — then the framing isn't just _the other ecology_. It's _the ecology that wakes when ours sleeps_. Every winter solstice tradition in the world says the same thing about the dark months: thin places, the dead returning, the veil drawing back. We don't have to invent that resonance. It's there in every winter the player has ever lived through.
>
> "Indifferent to Earth seasons" was vague and I knew it. _Inverse-phased_ is the right framing. Spring is the prairie's grip; winter is the loosening. The other ecology is most present at the moment ours is least present. That is the cosmology rendered as a calendar.
...
>
> One thing this _does_ break, and I think it breaks it the right way: the Torchbearer's burn window in #9 was "late dormancy → early thaw." That's _immediately after_ the egregoric winter peak. The controlled burn is no longer a routine succession tool. It's the medium-loop response to the cold. The burn is what the player does _because of_ what happened in the winter.
...
> **Delta:** Yes. _(beat)_ And inside the Revery — every winter in the accelerated time is a push. The end-of-Revery summary stops reading as one delta. It reads as a sequence: _line moved north in the third winter. Held through summer. Moved further in the fifth winter. Held._ The Revery becomes a story told in seasons.
...
> **Boon:** Annotating. One omen-weight parameter, gated on season-at-trigger. And one open question: does egregoric spread happen _at all_ during stewardship-phase winters, or only inside Revery winters?

> **Astrid:** A little. Below the threshold of micro-defense. A tile or two per stewardship winter, no more — the player notices when they return to a familiar area and it's different. Observable, not actionable. The Revery is where the real push happens; stewardship winter is the veil _breathing_.
...
> **Delta:** And the "Far Garden conversion" failure-state in #10 gets a clearer narrative shape. Not "the egregores won" abstractly. _The prairie didn't survive the winter._ A specific winter, multiple Reveries in a row where the summer rebound failed to undo the winter advance. The player can name when it happened — the winter the line crossed the village.
...
_The next morning, Delta opens with a question._

> **Delta:** What if the steward can actually help the egregoric biome?

> **Astrid:** _(immediate)_ That's the line the game has been quietly avoiding, and I'm glad you said it. The précis defines stewardship as a position — _steward, participant, observer, temporary influence_ — but it never says what the steward is for. We've assumed the prairie. The précis doesn't. Stewardship is a _role_, not an _allegiance_. Letting the player flip — or, more precisely, letting them _not be sure which side they're tending_ — deepens the moral axis enormously.
>
> But the danger is obvious. The easy version is a UI toggle: "plant egregores." That collapses everything. The egregores stop being not-of-this-Earth and become _another flora set_. We have to keep the unnaming and the cosmological otherness intact even while making help possible. Which means: help cannot be a button.
...
>
> 1. **Passive transmission.** Walking through egregoric tiles attaches drift particles to the player's footstep trail (`emitPlayerTrailBurst` already does the trail emission; add an egregoric variant for recent-egregoric-contact). The player is a carrier without knowing it. Some NPCs glance at them differently. The manual entry for _the player themselves_ may begin to glitch over many Reveries — single fields drifting into the egregore register.
> 2. **Refusal.** When the Torchbearer arrives at thaw, the player can dismiss them. The line that winter moved stays moved. Pure withholding — no new affordance, just the absence of an existing one. The most doctrine-aligned form of help.
> 3. **Meteorite-carrying.** Meteorites are already the visible carrier in the lore. The player can pick one up (`items.ts:26`) and _place_ it — not consume, not craft. Where they drop it, the veil thins; the next Revery seeds egregoric tiles nearby. The act is concrete but uncanny — the player is moving a star.
...
>
> Each layer has a different cost and a different weight. The player accumulates a vocabulary of _ways to lean_, from accidental to ceremonial.
...
> **Astrid:** And the player's entry glitches _toward the egregore register_ — not into it. Single fields drifting, the binomial line developing a stutter, one of the labeled fields filling with the egregore script. Never the full conversion in real time. The player _becomes_ the slow conversion. That is the late-game self-portrait, and it lands only if the player has been helping the egregores enough for it to make sense.

> **Calla:** This also gives us late-game NPC reactivity that costs almost nothing. Moab might say a different opening line if the player has been carrying meteorites. A ghost might say "you've been _out there_, haven't you" without specifying. The Torchbearer might pause before lighting, looking at the player, deciding whether to obey. None of these are systems — they're conditional dialog. But they make the player's accumulated leaning feel _witnessed_.

> **Delta:** Yes. And — the most important constraint — _the game never confirms the player has helped the egregores_. There's no progress bar, no allegiance meter, no end-screen tally. The player accumulates moments that may or may not have been help. The trail particles disperse silently. The placed meteorite looks the same as any other. The ritual, if performed, produces a small visual moment and nothing else. The game _registers_ the help internally, surfaces it through downstream effects (NPC reactivity, player-entry glitch, biased Revery spread), but never _names_ the registration. The player builds their own private model of "what I have done." That ambiguity is the moral weight.
...
> **Astrid:** And the failure-state biome — the Far Garden conversion in #10 — gets a third reading now. Before: _the prairie didn't survive the winter_ (passive). Just now: _the winter the line crossed the village_ (passive-with-date). New layer: _what the steward chose_. Three readings of the same biome. The player carries their own.

> **Delta:** Yes. The biome doesn't change. The player's relationship to it does.

> **Calla:** One last thing. The Torchbearer's refusal in layer 2 — when the player dismisses them — is the moment where help is least ambiguous. The NPC walked across the prairie to meet you at thaw, and you sent them away. That has to _land_. The Torchbearer doesn't argue. They look at the line, look at you, and turn back. The player watches them walk to the horizon. The line stays where the winter put it. That image is the medium-loop equivalent of the manual's refusal — the player's framework refusing what the world offered.
...
_That afternoon, Delta sends a link. Calla opens it._

> **Calla:** Delta sent us _voynich.nu/transcr.html_ — the transcription page. He's asking whether the egregoric script should literally be **Voynich**, not a Voynich-_style_ constructed alphabet.

> **Astrid:** _(immediate)_ Yes. The Voynich manuscript is a 15th-century naturalist's journal in an undeciphered script, and its largest section is the _herbal_ — page after page of illustrated plants nobody can identify. _That is the egregoric manual entry._ The manuscript is already what we're trying to make. Using a constructed alphabet was honoring the reference. Using the actual script _is_ the reference. We'd be foolish not to.
>
...
> **Calla:** And ~280 glyphs is exactly the variety we wanted for "stable per tile, varied across tiles." Each egregoric tile's manual page samples deterministically from that set — different sample per tile, same sample every time the page is reopened. The character distribution will read as _a language_ because, structurally, it is one. The Voynich's repetition patterns (the famous "daiin daiin daiin" runs) are part of the texture; we don't have to fake the linguistic plausibility.

> **Astrid:** And the readable-word slips — _threshold, between, garden_ — stay in Latin. The contrast between Voynich script and an English word _in the same paragraph_ is enormous. The English word feels like it's been _uncovered_ in the Voynich text. Like the manual occasionally pierces.
...
> **Astrid:** Using _actual transcribed Voynich tokens_ means the manual entries contain real, attested Voynich "words" — strings that have been argued over for decades, that researchers have catalogued and analyzed. The player is looking at _the same characters the world's most famous undeciphered text contains_. Some of those tokens are weighted with their own scholarship. We don't surface that. We just use them.

> **Calla:** One careful thing: the Voynich is a real cultural artifact and we should credit it. Manual entries in egregore register don't _say_ "Voynich" — that breaks the unnaming. But the game's credits screen lists the font, the EVA project (Zandbergen & Landini), and gestures at the manuscript as the source. The reverence stays where it belongs.

> **Delta:** Yes. And — one more thing. The Voynich herbal pages have _illustrations_. Plants we can't identify, drawn in ink. We can't replicate the manuscript's full visual style, but the egregoric tile glyph on the world map should be a single Voynich character — chosen from a small consistent subset — that the player learns to recognize. The script in the manual is the _text_; the tile is the _frontispiece_. One coherent visual register.

> **Boon:** One tile glyph per egregoric variant in 8b, all drawn from the same Voynich subset. Annotating.

> **Astrid:** This is the strongest the unnaming has ever been. The script is real. The book exists. The plants in it have never been named. The player is leafing through a fragment of _the_ untranslatable naturalist's journal, and finding what the prairie became.
...
_Astrid stays late at the office. The next morning she pulls everyone back in._
...
> **Astrid:** The game has _always_ rendered in characters. Clover is `%`. Bees are `*`. Seeds are `·`. The map is typographic; the inventory is typographic; the manual is typographic. The world has been a text the entire time. What we're proposing — egregoric tiles in Voynich, egregoric manual entries in Voynich — isn't "different glyphs for different things." It's a _different writing system_. ASCII and Voynich aren't two character sets sharing a tile. They are two _scripts_, in the linguistic sense. The player isn't being asked to recognize a new symbol. They're being asked to read in two languages.

> **Boon:** _(slowly)_ Right. And the structural claim under that is **the character set is the ontology**. There is no separation between what the world is made of and what we use to describe it. ASCII tiles in the world; ASCII text in the manual; ASCII in recipes, inventory, HUD, dialog. Native flora and the entire game UI are encoded in the same script. Voynich tiles in the world; Voynich text in their manual entries. The medium isn't a window onto the cosmology. The medium _is_ the cosmology.

> **Astrid:** And ASCII is _Earth_. Not metaphorically. ASCII is a Western, post-Enlightenment, technical encoding standard defined in 1963 by a committee in Manhattan. It is the lingua franca of computers — which is to say, the alphabet of the world the player is sitting in right now. They have been reading ASCII their entire computing life. They have never thought of it as terrestrial. Until this game says: the prairie is rendered in your alphabet. The other ecology is not.

> **Calla:** _(quietly)_ That is the cleanest framing of the unnaming we have ever had. The egregoric flora isn't refusing to be named in our content. It's refusing to be encoded in our medium. The script the player has known their whole life — the script their phone and laptop and game world are built from — does not extend to these tiles. There is no ASCII codepoint for what they are. The boundary between Earth and not-Earth is **one byte wide**.

> **Boon:** And it's not invented for the game. ASCII is what the world's information actually runs on. Voynich is — depending on the theory — a hoax, a glossolalia, a forgotten language, a constructed cipher, a botanical record in an unrecovered script. Whatever it is, it has existed for six hundred years as _the_ example of a real human writing system that no Earth alphabet maps to. We are not constructing the boundary. The boundary is in the history of writing.

> **Astrid:** This is what the précis has been pointing at the whole time and I didn't see it until now. The naturalist's manual isn't a _separate_ lens. The manual is **the same medium the prairie is made of, attempting to describe itself**. ASCII looking at ASCII. When the egregoric flora appears, the script the player is reading in is no longer the script the thing is written in. That is the failure of the framework, rendered as a failure of _encoding_.
...
> **Boon:** End-of-Revery summary is the obvious surface. Native changes render in ASCII — "the clover patches near the village expanded northward through the third summer." Egregoric changes render in _Voynich_ — EVA token strings for "what arrived," typographically separated from the native prose. The summary is **bilingual in the literal sense**. The seasonal narrative tells itself in two scripts.

> **Calla:** And the player's _name_, over the long arc. We said earlier that the player's manual entry glitches toward the egregore register when they've been helping the egregores. Push further: their **name drifts character by character**. Single ASCII letters replaced with Voynich equivalents. Not all at once — one letter, then two, then three, over many Reveries of accumulated `cosmologicalDrift`. The player's name _fails to settle in ASCII_. They are typographically pierced.

> **Astrid:** That is the late-game self-portrait, made of script. Not "the player has changed" as a story event. Their _name itself_ no longer renders in their alphabet. They have been reading Voynich for a long time. The reading has done something.

> **Boon:** Recipes too — once #12 exists. A recipe that bridges native and egregoric ingredients renders with glyphs from both scripts adjacent in the entry. The recipe is _visibly liminal_. We don't label such recipes "crossbreed" or "hybrid." The typography does it.
...
> **Calla:** And it never confirms — never says "you have crossed over" — but the manual entry for the player, once illegible in part, is no longer purely ASCII. The page is _itself_.

> **Delta:** _(quietly)_ That is the game.
...
_Two days later. Astrid hasn't slept much._

> **Astrid:** I want to push round six harder. We said we'd _embed_ a Voynich font — pick an existing one off the shelf. But Boon's round-seven framing changes the question. If the character set is the ontology, the typeface isn't an asset. It's the substrate. So: **what if we ship our own typeface with the game?** Designed for it. The face the game reads in.

> **Boon:** Concretely: one monospace face, containing the standard ASCII range _and_ Voynich glyphs in PUA at the same codepoints we locked in round six (`U+FF400`–`U+FF51F`). Same hand. Same stroke weight, same proportions, same terminals. The Latin characters and the Voynich characters look like cousins because **the same designer drew them**.

> **Astrid:** And the two scripts should look like they were drawn by the _same naturalist's hand_. The prairie's clover and the egregoric flora both grow on the same fragment of Earth. They share the page. The typography should reflect that. The Voynich glyphs shouldn't look alien — they should look like _Latin, slightly twisted_. The way actual Voynich already does, naturally. We push it further: design the entire face so the two scripts feel like one writing system telling two stories.

> **Calla:** That makes the boundary one of **content, not appearance**. The cosmological line isn't a visual line — it's a semantic line. Players read across it without their eye registering a different script. They register a different _intention_. That is so much subtler and so much stronger than "Voynich looks weird." The eye sees one alphabet; the mind catches that the meanings refuse to map.
...
> **Calla:** **Tunic** is the closest precedent — a game shipping a constructed alphabet the player learns to read as part of the experience. Ours is meant to _stay_ partly unreadable. Tunic eventually decipherable; ours ontologically un-decipherable. But the typographic ambition is the same. Players will recognize "this game" by how it reads. That is enormous identity work without anything that _looks like_ branding.
>
...
- **Binomial line** (genus + species position): 1 medium token + 1 medium-or-long token. E.g. _daiin chedy_ in Dialect B; _daiin chol_ in Dialect A. Renders as if it were a Latin binomial but isn't.
...
7. **The character set is the ontology (post-approval amendment, see Round 7 in #8).** ASCII is Earth's encoding; Voynich is the example of a real human script that no Earth alphabet maps to. The medium isn't a window onto the cosmology — the medium _is_ the cosmology. Concrete surface expansions beyond #6's tile + manual entries:
   ...
   - **Player-name rendering** drifts character by character over accumulated `cosmologicalDrift` (Round 5). Single ASCII letters replaced with Voynich equivalents at threshold steps — one, then two, then three. The player's name _fails to settle in ASCII_ in the late game. Implementation: per-character font switching in the React renderer (span treatment with `font-family: voynich`).
   - **Player's own manual entry** gains a single readable English word from the egregoric allowlist (`threshold`, `between`, _here_ …) once name-drift exceeds threshold. Mirrors the egregoric entries' Latin pierces, inverted (Latin appearing in an ASCII page). The cosmology recognizes the player back.
   - **Recipes crossing the boundary** (post-#12) render with glyphs from both scripts adjacent in the entry. The recipe is _visibly liminal_; no "crossbreed" label needed — typography does the work.
