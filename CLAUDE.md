...
- `egregore` — egregoric flora tile (Voynich glyph from `EGREGORE_GLYPHS`, color `#7A88A0`). Walkable, inert in precis #8a (no lifecycle, no interaction). Tile glyph is per-position-deterministic; the renderer applies the `'Voynich'` font family for the draw call only. See `## egregores` below.
...
npm run precis          # terminal kanban for the precis sequencing (see "precis dashboard")
```
...
`npm run precis` launches it. lives at `tools/precis/` (Ink TUI, run via `tsx`). dev-only — not bundled with the game, not in any tsconfig project, eslint-ignored.

when picking up a precis feature:

1. open the dashboard: `npm run precis`. the NEXT column shows everything whose dependencies are all `shipped`.
