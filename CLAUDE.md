...
- **new ECS entity creation** — route to the target zone's world via `getWorldForZone(state, zone, ruinIndex?)` from `src/engine/zone.ts`, then call `world.createEntity()` + `world.addComponent(...)`. NEVER write `addComponent(eid, ComponentType.EntityZone, ...)` — the component does not exist; an entity's zone is the world it lives in. `state.world` is a getter that always resolves to the active zone's world; use it for the common case where the entity belongs to the current zone. For the rare cross-zone read (Moab while in overworld, etc.), use `queryAllZones(state, ...types)` or `getWorldForZone(state, otherZone)`. Cross-zone entity movement uses `moveEntityAcrossWorlds(sourceWorld, sourceEid, targetWorld)` — copies all components, destroys the source, returns a new eid (eids are per-world). → `docs/claude/state.md`
