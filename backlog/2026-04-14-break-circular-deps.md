...
1. `movement.ts` -> `cave.ts` -> `coyote.ts` -> `movement.ts`. fix by extracting `getBlockedPositions` into `blocking.ts` — it's a pure query function that reads omnibox/entity positions and has no dependency on cave or coyote logic.
