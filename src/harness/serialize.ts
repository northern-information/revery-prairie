 ...
 */
export const serializeState = (state: GameState): string => JSON.stringify(state, serializeReplacer, 2)

/**
 * Deserialize a JSON string back to a GameState-shaped object.
 * Maps and Sets are restored. Functions remain null.
 */
export const deserializeState = (json: string): GameState => JSON.parse(json, deserializeReviver) as GameState
 ...
 */
export const FUNCTION_FIELDS: (keyof GameState)[] = ['previewFn', 'pendingAction', 'world', 'genesis', 'onPlayerMoved']
