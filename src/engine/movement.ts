...
export const movePlayer = (state: GameState, dir: Direction): boolean => {
  ...
  // RP-49 — cube-step gate. Adjacent tiles whose elevation tier
  // differs by more than 1 are unclimbable, even when both tiles
  // are walkable by type. Cave/ruin zones have no elevation
  // entries, so isClimbableStep returns true there.
  ...
  if (!isClimbableStep(state.elevation, state.player.x, state.player.y, nx, ny, frozenStairways)) {
    updateFacingEntity(state)
    return false
  }
  ...
  if (isDiagonalDirection(dir)) {
    ...
    if (
      !isClimbableStep(state.elevation, state.player.x, state.player.y, cx, cy, frozenStairways) ||
      !isClimbableStep(state.elevation, state.player.x, state.player.y, cx2, cy2, frozenStairways)
    ) {
      updateFacingEntity(state)
      return false
    }
