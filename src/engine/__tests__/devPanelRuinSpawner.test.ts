...
describe('dev panel ruin spawner', () => {
  ...
  describe('spawnDevRuin happy path', () => {
    it('places a RuinEntrance tile at the drop position', () => {
      ...
      const ok = spawnDevRuin(state, { x: 5, y: 6 }, RuinArchetype.Subsidence, 'Δ')
      expect(ok).toBe(true)
    ...
    it('appends a RuinInterior with matching entranceOverworld and ruinIndex', () => {
      const state = makeOverworldState()
      spawnDevRuin(state, { x: 5, y: 6 }, RuinArchetype.Subsidence, 'Δ')
    ...
    it('stores the chosen explicit glyph on the interior', () => {
      const state = makeOverworldState()
      spawnDevRuin(state, { x: 5, y: 6 }, RuinArchetype.Subsidence, 'Φ')
    ...
    it('resolves "random" glyph to a member of ENTRANCE_GLYPHS at drop time', () => {
      ...
      const state = makeOverworldState()
      spawnDevRuin(state, { x: 5, y: 6 }, RuinArchetype.Subsidence, RUIN_GLYPH_RANDOM)
    ...
    it('assigns ruinIndex sequentially across multiple drops', () => {
      const state = makeOverworldState()
      spawnDevRuin(state, { x: 3, y: 3 }, RuinArchetype.Subsidence, 'Δ')
      spawnDevRuin(state, { x: 8, y: 8 }, RuinArchetype.Resonance, 'Φ')
  ...
  describe('drop validation (silent reject)', () => {
    it('rejects out-of-bounds drops', () => {
      const state = makeOverworldState()
      expect(spawnDevRuin(state, { x: -1, y: 5 }, RuinArchetype.Subsidence, 'Δ')).toBe(false)
      expect(spawnDevRuin(state, { x: 5, y: 99 }, RuinArchetype.Subsidence, 'Δ')).toBe(false)
    ...
    it('rejects drops on Space tiles', () => {
      ...
      expect(spawnDevRuin(state, { x: 5, y: 5 }, RuinArchetype.Subsidence, 'Δ')).toBe(false)
