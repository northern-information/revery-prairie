...
export interface InFlightScan {
  ...
  // Worktrees doing thinktank-only doc work — no backlog id derivable.
  ...
  stale: string[]
  // ids whose branch matches `RP-N-` but the worktree has no internal-scan
...
// Match branch names that encode a backlog id.
// Forms supported:
//   RP-33-slug
//   worktree-RP-33-slug
//   backlog/RP-33-shipped
const BACKLOG_BRANCH_RE = /(?:^|\/|-)RP-(\d+[a-z]?)(?:-|$)/
const SPEC_FILE_RE = /^harness\/specs\/(RP-\d+[a-z]?)-/
const PLAN_FILE_RE = /^harness\/plans\/(RP-\d+[a-z]?)-/
const THINKTANK_FILE_RE = /^docs\/backlog-thinktank-v\d+/

const extractBacklogId = (name: string): string | null => {
  const m = BACKLOG_BRANCH_RE.exec(name)
...
const scanWorktreeFiles = (worktreePath: string, warnings: string[]): WorktreeScan => {
  ...
  for (const p of paths) {
    ...
    if (p === 'docs/backlog.yaml') yamlChanged = true
    if (THINKTANK_FILE_RE.test(p)) thinktank.push(p)
  }

  let yamlFlips: { id: string; from: Status; to: Status }[] = []
  if (yamlChanged) {
    const mainYaml = run('git', ['-C', worktreePath, 'show', 'main:docs/backlog.yaml'])
    if (mainYaml.ok) {
      const mainStatuses = parseStatuses(mainYaml.stdout)
      try {
        const wtPath = resolve(worktreePath, 'docs/backlog.yaml')
...
export const scanInFlight = (startDir: string, features: Feature[]): InFlightScan => {
  ...
  for (const wt of worktrees) {
    ...
    if (scan.thinktank.length > 0) {
      const branchId = wt.branch ? extractBacklogId(wt.branch) : null
  ...
  // Branch-only: a backlog id matches a worktree/branch/PR by name, but no
  ...
  // (e.g. an RP-23 worktree happens to also flip RP-18's status) don't
  // mask the fact that RP-23 itself has no harness work.
