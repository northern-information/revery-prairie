...
export interface MoveParams {
  id: string
  newStatus: Status
  yamlRelPath: string // e.g. 'docs/precis-status.yaml'
...
export const moveFeatureAndOpenPr = (params: MoveParams): MoveResult => {
  ...
  try {
    ...
    const branch = `backlog/precis-${sanitize(params.id)}-${params.newStatus}`
    ...
    if (next === raw) {
      ...
      return {
        ok: false,
        error: `precis-${params.id} is already ${params.newStatus}. Nothing to change.`,
        log,
      }
    }
    ...
    const title = `Maintain backlog: move precis-${params.id} to ${params.newStatus}`
    ...
    // Draft PR.
    const body = `Moves precis-${params.id} to \`${params.newStatus}\` in \`${params.yamlRelPath}\`.\n\nOpened from the \`npm run backlog\` TUI.`
